...
<aui:form action="<%= configurationActionURL %>" method="post" name="fm" onSubmit="event.preventDefault();">
	...
	<c:if test="<%= typeSelection.equals(StringPool.BLANK) %>">
		...
		<c:choose>
			...
			<c:otherwise>
				<aui:select label="asset-selection-type" name="preferences--selectionStyle--" onChange='<%= renderResponse.getNamespace() + "chooseSelectionStyle();" %>'>
		...
		<c:choose>
			...
			<c:when test='<%= selectionStyle.equals("dynamic") %>'>
				<liferay-ui:panel-container extended="<%= true %>" id="assetPublisherDynamicSelectionStylePanelContainer" persistState="<%= true %>">
					...
					<liferay-ui:panel collapsible="<%= true %>" extended="<%= true %>" id="assetPublisherQueryRulesPanelContainer" persistState="<%= true %>" title="asset-filter">
						...
						<aui:fieldset>
							<span class="aui-field-row">
								<aui:select inlineField="<%= true %>" inlineLabel="left" label="order-by" name="preferences--orderByColumn1--">
									...
									<aui:option label="priority" selected='<%= orderByColumn1.equals("priority") %>'/>
									<aui:option label="view-count" selected='<%= orderByColumn1.equals("viewCount") %>' value="viewCount" />
									<aui:option label="ratings" selected='<%= orderByColumn1.equals("ratings") %>'/>
								</aui:select>
					...
					<liferay-ui:panel collapsible="<%= true %>" extended="<%= true %>" id="assetPublisherRelatedAssetPanel" persistState="<%= true %>" title="related-assets">
						<%@ include file="/html/portlet/asset_publisher/related_assets_settings.jspf" %>
					</liferay-ui:panel>
