...
import com.liferay.portlet.journal.util.comparator.ArticleAuthorComparator;
...
import com.liferay.portlet.journal.util.comparator.ArticleStatusComparator;
 ...
 * @author Hugo Huijser
 * @author HUNGDX - Them order bai viet theo Status,Title va Author
 */
public class JournalUtil {
	...
	/**
	 * @todo Them order hien thi danh sach bai viet theo Status,User
	 * @author HUNGDX	
	 * @date Nov 22, 2012
	...
	public static OrderByComparator getArticleOrderByComparator(
		...
		if (orderByCol.equals("create-date")) {
			...
			orderByComparator = new ArticleAuthorComparator(orderByAsc);
		}
	...
	public static Stack<JournalStructure> getRecentStructures(
		...
		if (recentStructures == null) {
			recentStructures = new FiniteUniqueStack<JournalStructure>(MAX_STACK_SIZE);
	...
	synchronized static void _populateCustomTokens(Map<String, String> tokens) {
	...
	private static void _removeOldContent(
		...
		StringBuilder elementPathBuilder = new StringBuilder();
		
		for (int i = 0; i < path.size(); i++) {
			elementPathBuilder.append("/" + path.elementAt(i));
		}

		elementPath = elementPathBuilder.toString();
