...
public class DocumentCommandReceiver extends BaseCommandReceiver {
	...
	@Override
	/**
	 * HUngDX sua ngay 20/01/2014
	 * Noi dung: Thay doi duong dan anh
	 */
	/**
	 * AnhBDb sua ngay 20/05/2015
	 * Noi dung: Sap xep list fileentries theo createDate
	 */
	...
	private void _getFiles(
		...
		Group group = commandArgument.getCurrentGroup();
		OrderByComparator comparator = OrderByComparatorFactoryUtil.create("dlfileentry", "createDate", false);

		Folder folder = _getFolder(
			group.getGroupId(), commandArgument.getCurrentFolder());

		List<FileEntry> fileEntries = DLAppServiceUtil.getFileEntries(folder.getRepositoryId(), folder.getFolderId(), -1, -1, comparator); 
//				getFileEntries(
//			folder.getRepositoryId(), folder.getFolderId(), comparator);
...
//			
//			String fileEntryIdStr = String.valueOf(fileEntryId);
//			
//			String[] fileEntryIdSplitedArr = fileEntryIdStr.split("(?<=\\G.{2})");
//			
//			sb.append(_THUMBNAIL_FILTER_PATH);
//			sb.append(_THUMBNAIL_DEFAULT_SIZE);
//			sb.append(StringPool.SLASH);
//			
//			for (int i = 0; i < fileEntryIdSplitedArr.length; i++){
//				
//				sb.append(fileEntryIdSplitedArr[i]);
//				sb.append(StringPool.SLASH);
//			}
//			
//			sb.append(fileEntry.getFileEntryId() + "." + fileEntry.getExtension());
	...
	private final String STATIC_CACHE_DIR = PropsUtil.get("static.cache.dir");
	
	private static final String _THUMBNAIL_FILTER_PATH = "/thumbnail/";
	
	private static final String _THUMBNAIL_DEFAULT_SIZE = "600x400";
	
	private final String STATIC_DOCUMENTS_FOLDER_NAME = PropsUtil.get("static.documents.folder.name");
