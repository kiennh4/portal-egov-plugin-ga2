	...
	public void processAction(
		...
		try {
			...
			if (cmd.equals(Constants.ADD)) {
				 ...
				 */
				//Check limit user count by license
				int licenseUserLimit = PortalEgovLicenseUtil.getMaxUsers();
				
				if(licenseUserLimit > 1){//acept admin user
					
					long companyId = PortalUtil.getCompanyId(actionRequest);
					int userCounter = UserLocalServiceUtil.getCompanyUsersCount(companyId);
					
					if(userCounter >= licenseUserLimit){
						throw new PortalLicenseException("User limited by license");
					}
				}
