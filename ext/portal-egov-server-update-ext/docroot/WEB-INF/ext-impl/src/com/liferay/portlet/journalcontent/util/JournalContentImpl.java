...
//			languageId, page, secure);
		...
		String key = memcachedEncodeKey(groupId,articleId,version,templateId);
		
//		JournalArticleDisplay articleDisplay =
//			(JournalArticleDisplay)portalCache.get(key);
		
		byte[] articleDisplayByte = (byte[])MemcachedUtil.getInstance().get(key);
		
		JournalArticleDisplay articleDisplay  = null;
		
		if (articleDisplayByte!=null)
		
			articleDisplay = (JournalArticleDisplay) SerializationUtils.deserialize(articleDisplayByte);
		...
		if ((articleDisplay == null) || !lifecycleRender) {
			...
			if ((articleDisplay != null) && articleDisplay.isCacheable() &&
				...
				try {
					MemcachedUtil.getInstance().set(key, SerializationUtils.serialize(articleDisplay));
				}
				catch (Exception e)
				{
					e.printStackTrace();
...
//				portalCache.put(key, articleDisplay);
	...
	protected String memcachedEncodeKey (long groupId, String articleId, double version, String templateId) {
