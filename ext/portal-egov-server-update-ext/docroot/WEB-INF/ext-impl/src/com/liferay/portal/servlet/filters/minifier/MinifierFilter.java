...
public class MinifierFilter extends BasePortalFilter {
	...
	public static String aggregateCss(String dir, String content) throws IOException{
		...
		while (true){
			...
			if ((importX == -1) || (importY == -1)){
				...
				String importFullFileName = dir.concat(StringPool.SLASH).concat(importFileName);
				...
				if (importContent == null) {
					...
					if (_log.isWarnEnabled()) {
						_log.warn("File " + importFullFileName + " does not exist");
					}
	...
	protected Object getMinifiedBundleContent(HttpServletRequest request, HttpServletResponse response)
		throws IOException {
		...
		if (Validator.isNull(minifierType) ||
			...
			!ArrayUtil.contains(PropsValues.JAVASCRIPT_BUNDLE_IDS, minifierBundleId)){
		...
		// static cache
		if(Validator.isNotNull(_CACHE_DIR)){
			
			String requestURI = request.getRequestURI();
			requestURI = requestURI.replace(".jsp", ".js");
			String staticCacheDir = _CACHE_DIR.concat(requestURI);
			FileUtil.write(staticCacheDir, minifiedContent);
		}
	...
	protected String minifyCss(HttpServletRequest request, HttpServletResponse response, File file)
		throws IOException {
	...
	//private static final String _CACHE_DIR = "D:/cache";
	
	private static final String _CACHE_DIR = PropsUtil.get("static.cache.dir");
