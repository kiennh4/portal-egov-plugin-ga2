...
boolean anyAssetType = GetterUtil.getBoolean(preferences.getValue("anyAssetType", Boolean.FALSE.toString()));

//Hook by HungDX 08/08/2012 Sua kieu asset mac dinh la Webcontent 
long webContentClassNameId = ClassNameLocalServiceUtil.getClassNameId(JournalArticle.class);
long[] classNameIds = AssetPublisherUtil.getClassNameIds(preferences, new long[]{webContentClassNameId});
...
//Hook by anhbdb, check topic categories param

if (request.getParameter("topicCategoryIds")!=null)
{
	assetCategoryId = 0L;
	
	String topicCategoryIds = ParamUtil.getString(request, "topicCategoryIds");
	
	if (topicCategoryIds.equals(""))
	{
		assetEntryQuery.setVisible(false);
	}
	else
	if (Validator.isNotNull(topicCategoryIds))
	{
		String[] categoryIdsStr = topicCategoryIds.split(",");
		
		
		if ((Validator.isNotNull(categoryIdsStr))&&(categoryIdsStr.length>0))
		{
		
			long[] categoryIds = new long[categoryIdsStr.length];
			
			for (int i = 0; i < categoryIdsStr.length;i++)
			{
				categoryIds[i] = Long.valueOf(categoryIdsStr[i]);
			}
			
			if ((Validator.isNotNull(categoryIds))&&(categoryIds.length>0))
			{
...
int abstractLength = GetterUtil.getInteger(preferences.getValue("abstractLength", null), 0);
...
boolean showMetadataDescriptions = GetterUtil.getBoolean(preferences.getValue("showMetadataDescriptions", "true"), true);
...
boolean enablePrint = GetterUtil.getBoolean(preferences.getValue("enablePrint", "true"));
boolean enableFlags = GetterUtil.getBoolean(preferences.getValue("enableFlags", null));
boolean enableSocialBookmarks = GetterUtil.getBoolean(preferences.getValue("enableSocialBookmarks", null), false);
String socialBookmarksDisplayStyle = preferences.getValue("socialBookmarksDisplayStyle", "simple");
String socialBookmarksDisplayPosition = preferences.getValue("socialBookmarksDisplayPosition", "top");
...
//Hien thi noi dung mo ta ngan cung voi noi dung chi tiet bai viet
boolean displayAssetDescription = GetterUtil.getBoolean(preferences.getValue("displayAssetDescription", "true"));

//Hien thi nhung tin bai lien quan
boolean displayRelatedAsset = GetterUtil.getBoolean(preferences.getValue("displayRelatedAsset", "true"));

//Tieu chi loc tin bai lien quan,mac dinh la loc theo category cua tin bai hien tai dang hien thi full content
String relatedAssetsFilter = preferences.getValue("relatedAssetsFilter", "assetCategory");

//So luong tin bai lien quan hien thi tren trang
int numbersOfRelatedAssetsDisplay = GetterUtil.getInteger(preferences.getValue("numbersOfRelatedAssetsDisplay", "6"));

//Hien thi publish date cua tin bai lien quan ?
boolean displayRelatedAssetPublishDate = GetterUtil.getBoolean(preferences.getValue("displayRelatedAssetPublishDate", "true"));
%>
