...
import com.liferay.portal.memcached.MemcachedUtil;
 ...
 * @author hungdx - Sua lai phan upload anh trong method updateArticle
 * @author AnhBDb - Them updateMemcached phan updateArticle, addArticle
 */
public class JournalArticleLocalServiceImpl
	...
	public JournalArticle addArticle(
		...
		//Update Journal Article Memcached
		updateMemcachedArticleEntity (groupId, article.getArticleId(),article.getVersion(),article.getTemplateId());
		...
		try {
			if (tagNames != null) {
				...
				if (tagNames.length == 0) {
					...
					listTag = new ArrayList<String>();
					for (String itemTag : tagNames) {
						listTag.add(itemTag);
					}
				}
				if (!listTag.contains(NEW_TAG)) {
					listTag.add(NEW_TAG);
				}
				String[] tag_Names = new String[listTag.size()];
				for(int i = 0; i < listTag.size(); i++){
					tag_Names[i] = listTag.get(i);
				}
				
				// Update memcached
				updateMemcached(JournalArticle.class.getName(), classPK,
						groupId, serviceContext.getAssetCategoryIds(),
						tag_Names);

				// Asset
				updateAsset(userId, article,
						serviceContext.getAssetCategoryIds(), tag_Names,
			...
			// Update memcached
			updateMemcached(JournalArticle.class.getName(), classPK, groupId,
	...
	public JournalArticle copyArticle(
		...
		//Update Journal Article Memcached
		updateMemcachedArticleEntity (groupId, newArticle.getArticleId(),newArticle.getVersion(),newArticle.getTemplateId());
			
		//Update memcached
		updateMemcached (JournalArticle.class.getName(), oldArticle.getResourcePrimKey(), groupId, assetCategoryIds,
				assetTagNames);
	...
	public void deleteArticle(
		...
		//Update memcached
		updateMemcached (JournalArticle.class.getName(), article.getResourcePrimKey()
				, article.getGroupId(), serviceContext.getAssetCategoryIds(),
				serviceContext.getAssetTagNames());
...
//		String key = JournalArticleDisplay.class.getName()+"_"+article.getArticleId();
//		
//		try
//		{
//			if (article!=null)
...
//				getArticleD
//				
//				new JournalArticleDisplayImpl(
//						article.getCompanyId(), article.getId(),
//						article.getResourcePrimKey(), article.getGroupId(),
//						article.getUserId(), article.getArticleId(), article.getVersion(),
//						article.getTitle(defaultLanguageId), article.getUrlTitle(),
//						article.getDescription(defaultLanguageId), article.getAvailableLocales(),
//						content, article.getType(), article.getStructureId(), templateId,
//						article.isSmallImage(), article.getSmallImageId(),
//						article.getSmallImageURL(), numberOfPages, page, paginate,
//						cacheable);
//				
//				MemcachedUtil.getInstance().set(key, SerializationUtils.serialize(article));
//			}
//		}
//		catch (Exception e)
//		{
...
//		}
		...
		//Update memcached
				updateMemcached (JournalArticle.class.getName(), oldArticle.getResourcePrimKey(), groupId, serviceContext.getAssetCategoryIds(),
						serviceContext.getAssetTagNames());
	...
	/**
	 * updateMemcachedArticleEntity
	 * @param groupId
	 * @param articleId
	 * @param version
	 * @param templateId
	 */
	public void updateMemcachedArticleEntity (long groupId, String articleId, double version, String templateId) 
	{
		if (version>0)
		{
			if (MemcachedUtil.getInstance().get(encodeKeyArticleEntity(groupId, articleId, version, templateId))!=null)
			{
				MemcachedUtil.getInstance().delete(encodeKeyArticleEntity(groupId, articleId, version, templateId));
			}
		}
		
		if (MemcachedUtil.getInstance().get(encodeKeyArticleEntity(groupId, articleId, 0.0, templateId))!=null)
		{
			MemcachedUtil.getInstance().delete(encodeKeyArticleEntity(groupId, articleId, 0.0, templateId));
		}
		
	}
	
	private String encodeKeyArticleEntity (long groupId, String articleId, double version, String templateId)
	{
	 ...
	 * @param assetTagNames
	 */
	public void updateMemcached (String className, long classPK, long groupId, long[] assetCategoryIds,
			String[] assetTagNames)
	{
		StringBundler sb = new StringBundler();
		
		try {
			AssetEntry oldAsset = AssetEntryLocalServiceUtil.fetchEntry(className, classPK);
			
			long[] oldAssetCategoryIds = new long[0];
			
			List<AssetTag> oldAssetTag = new ArrayList<AssetTag>();
			
			if (oldAsset!=null)
			{
			
				oldAssetCategoryIds = oldAsset.getCategoryIds();
				
				oldAssetTag = oldAsset.getTags();
			...
			int assetCategorySize = 0;
			
			if (assetTagNames!=null)
			{
				assetTagSize = assetTagNames.length;
			}
			
			if (assetCategoryIds!=null)
			{
				assetCategorySize = assetCategoryIds.length;
			}
			
			long[] assetTagIds = new long[assetTagSize+oldAssetTag.size()];
			
			int index = 0;
			
			if ((assetTagSize>0)||((oldAssetTag!=null)&&(oldAssetTag.size()>0)))
			{
				if (assetTagNames!=null)
				{
					for (String tagName : assetTagNames)
					{
						AssetTag tag = AssetTagLocalServiceUtil.getTag(parentGroupId, tagName);
						if (tag!=null)
						{
							assetTagIds[index] = tag.getTagId();
							index++;
						}
					}
				}
			...
			if ((assetCategorySize>0)||(index>0)||(oldAssetCategoryIds.length>0))
			{
				
				String[] listMemcachedKey = new String[0];
				
				if (SystemProperties.get(_SYSTEM_QUERY_KEY)!=null)
				{
					listMemcachedKey = SystemProperties.get(_SYSTEM_QUERY_KEY).split(StringPool.COMMA);
				}
				if ((listMemcachedKey!=null)&&(listMemcachedKey.length>0))
				{
					for (int i = 0; i <listMemcachedKey.length ; i++)
					{
						boolean deleteKey = false;
						
						String[] memcachedKeyIds = listMemcachedKey[i].split(StringPool.UNDERLINE)[0].split(StringPool.MINUS);
						
						if ((memcachedKeyIds!=null)&&(memcachedKeyIds.length>0))
						{
							if (assetCategoryIds!=null)
							{
								for (long categoryId : assetCategoryIds)
								{
									for (String memcachedKeyID : memcachedKeyIds)
									{
										if (memcachedKeyID.equals(StringUtil.valueOf(categoryId)))
										{
											MemcachedUtil.getInstance().delete(listMemcachedKey[i]);
											deleteKey = true;
											System.out.println("Delete key: "+listMemcachedKey[i]);
											break;
										}
									}
							...
							if ((!deleteKey)&&(oldAssetCategoryIds!=null)&&(oldAssetCategoryIds.length>0))							{
								for (long oldCategoryId : oldAssetCategoryIds)
								{
									for (String memcachedKeyID : memcachedKeyIds)
									{
										if (memcachedKeyID.equals(StringUtil.valueOf(oldCategoryId)))
										{
											MemcachedUtil.getInstance().delete(listMemcachedKey[i]);
											deleteKey = true;
											System.out.println("Delete key: "+listMemcachedKey[i]);
							...
							if (!deleteKey)
							{
								for (long tagId : assetTagIds)
								{
									for (String memcachedKeyID : memcachedKeyIds)
									{
										if (memcachedKeyID.equals(StringUtil.valueOf(tagId)))
										{
											MemcachedUtil.getInstance().delete(listMemcachedKey[i]);
											deleteKey = true;
											System.out.println("Delete key: "+listMemcachedKey[i]);
							...
							if (!deleteKey)
							{
								sb.append(listMemcachedKey[i]);
								
								if ( i < listMemcachedKey.length-1)
								{
					...
					String systemQueryKeyTemp = sb.toString();
					
					if (systemQueryKeyTemp.endsWith(StringPool.COMMA))
					{
						systemQueryKeyTemp = systemQueryKeyTemp.substring(0, systemQueryKeyTemp.length()-1);
					}
					
					SystemProperties.set(_SYSTEM_QUERY_KEY, systemQueryKeyTemp);
				}
			}
		}
		catch (Exception e)
		{
			_log.error("Update memcached error"+e);
		}
	}
	...
	public JournalArticle updateStatus(
		...
		updateMemcached (JournalArticle.class.getName(), article.getClassPK(), article.getGroupId()
				, serviceContext.getAssetCategoryIds(),
				serviceContext.getAssetTagNames());
	...
	private static final String NEW_TAG = "tin m\u1EDBi";
