...
import com.liferay.portlet.assetpublisher.util.AssetEntryCache;
...
public class AssetPublisherUtil {
	...
	public static AssetEntryQuery getAssetEntryQuery(
		...
		for (int i = 0; true; i++) {
			...
			if (Validator.equals(queryName, "assetCategories")) {
				...
				assetCategoryIds = categoryValidate(assetCategoryIds); 
	...
	public static List<AssetEntryCache> getListAssetEntry (AssetEntryQuery assetEntryQuery)
	{
		List<AssetEntryCache> assetList = new ArrayList<AssetEntryCache>();
		
		try 
		{
			String queryKey = buildQueryKey(assetEntryQuery);
			...
			if (assetList == null || assetList.isEmpty()) {
				...
				for(AssetEntry assetEntry : queryList){
					...
					entryCache.setTitle(assetEntry.getTitle());
					entryCache.setPublishDate(assetEntry.getPublishDate());
					entryCache.setCreateDate(assetEntry.getCreateDate());
					entryCache.setModifiedDate(assetEntry.getModifiedDate());
					entryCache.setExpirationDate(assetEntry.getExpirationDate());
					entryCache.setPriority(assetEntry.getPriority());
					entryCache.setCategories(assetEntry.getCategories());
					entryCache.setTags(assetEntry.getTags());
					entryCache.setViewCount(assetEntry.getViewCount());
					entryCache.setClassName(assetEntry.getClassName());
	...
	private static ArrayList<AssetEntryCache> getCacheData(String queryKey) {
		...
		if ((byte[])MemcachedUtil.getInstance().get(queryKey)!=null)
		
			cacheData = SerializationUtils.deserialize((byte[])MemcachedUtil.getInstance().get(queryKey));
	...
	public static boolean updateCacheData(String queryKey, List<AssetEntryCache> assetList) {
		
		return MemcachedUtil.getInstance().set(queryKey, SerializationUtils.serialize((Serializable) assetList));
	}

	
	public static void updateSystemQueryKeys(String queryKey) {
	...
	public static boolean existQueryKey(String queryKey) {
	...
	public static String buildQueryKey(
			...
			long[] queryIds = new long[assetEntryQuery.getAnyCategoryIds().length+assetEntryQuery.getAnyTagIds().length];
			
			if (queryIds.length==0)
			{
				queryIds = new long[assetEntryQuery.getAllCategoryIds().length+assetEntryQuery.getAllTagIds().length];
				
				for (int i = 0; i < assetEntryQuery.getAllCategoryIds().length;i++)
				{
					queryIds[i] = assetEntryQuery.getAllCategoryIds()[i];
				}
				
				for (int i = 0; i <assetEntryQuery.getAllTagIds().length;i++)
				{
					queryIds[assetEntryQuery.getAllCategoryIds().length+i] = assetEntryQuery.getAllTagIds()[i];
				}
				
			}
			else
			{
				for (int i = 0; i < assetEntryQuery.getAnyCategoryIds().length;i++)
				{
					queryIds[i] = assetEntryQuery.getAnyCategoryIds()[i];
				}
				
				for (int i = 0; i <assetEntryQuery.getAnyTagIds().length;i++)
				{
					queryIds[assetEntryQuery.getAnyCategoryIds().length+i] = assetEntryQuery.getAnyTagIds()[i];
				}
			}
			...
			stringBuilder.append(assetEntryQuery.getOrderByCol1().toLowerCase());
			stringBuilder.append(StringPool.DASH);

			stringBuilder.append(assetEntryQuery.getOrderByType1().toLowerCase());
			stringBuilder.append(StringPool.UNDERLINE);
			
			stringBuilder.append(assetEntryQuery.getOrderByCol2().toLowerCase());
			stringBuilder.append(StringPool.DASH);

			stringBuilder.append(assetEntryQuery.getOrderByType2().toLowerCase());
			stringBuilder.append(StringPool.UNDERLINE);

			stringBuilder.append(assetEntryQuery.getStart());
			stringBuilder.append(StringPool.DASH);
			stringBuilder.append(assetEntryQuery.getEnd());
			stringBuilder.append(StringPool.UNDERLINE);
			
			stringBuilder.append(Arrays.toString(assetEntryQuery.getGroupIds()));
			stringBuilder.append(StringPool.UNDERLINE);
			
			stringBuilder.append(assetEntryQuery.getLinkedAssetEntryId());
			stringBuilder.append(StringPool.UNDERLINE);
			
			if (assetEntryQuery.getLayout()!=null)
			{
				stringBuilder.append(assetEntryQuery.getLayout().getPrimaryKey());
				stringBuilder.append(StringPool.UNDERLINE);
			}
			
			stringBuilder.append(assetEntryQuery.getClassTypeIds().toString());
			
			return stringBuilder.toString();
		}
	
	public static long[] categoryValidate(long[] categoryIds) {
		try {

			StringBuilder sb = new StringBuilder();
			
			String validatedCategoryIdsStr = "";
			
			int numValidateCategory = 0;
			
			for (long categoryId : categoryIds) {

				AssetCategory category = AssetCategoryLocalServiceUtil
						.fetchAssetCategory(GetterUtil.getLong(categoryId));

				if (category != null) {
					sb.append(String.valueOf(GetterUtil.getLong(categoryId)));
					sb.append(StringPool.COMMA);
					numValidateCategory++;
				}
			}

			validatedCategoryIdsStr = sb.toString();
			
			if(validatedCategoryIdsStr.endsWith(StringPool.COMMA)){
				validatedCategoryIdsStr = validatedCategoryIdsStr.substring(0, validatedCategoryIdsStr.length()-1);
			}
			
			long[] validatedCategoryIds = new long[numValidateCategory];
			
			if (numValidateCategory>0)
			{
				for (int i = 0; i < numValidateCategory;i++)
				{
					validatedCategoryIds[i] = Long.parseLong(validatedCategoryIdsStr.split(StringPool.COMMA)[i]);
				}
			}
	...
	public static String categoryValidate(String categoryIds) {
		try {
