...
public class ThemeHotDeployListener extends BaseHotDeployListener {
	...
	public void invokeUndeploy(HotDeployEvent hotDeployEvent)
		...
		try {
			...
			throwHotDeployException(hotDeployEvent, "Error unregistering themes for ", t);
		}
	}
	...
	protected void doInvokeDeploy(HotDeployEvent hotDeployEvent)
		...
		if (_log.isInfoEnabled()) {
			if (themeIds.size() == 1) {
				...
				_log.info(themeIds.size() + " themes for " + servletContextName + " are available for use");
			}
		}
		
		//Copy anh cua theme sang thu muc cache apache
		if(Validator.isNotNull(PropsUtil.get("static.cache.dir"))){
			
			//Xoa temp cache cua portal
			FileUtil.deltree(_TEMP_DIR);
			
			String sourceDirName = PropsUtil.get("auto.deploy.tomcat.dest.dir") + StringPool.SLASH + servletContextName + StringPool.SLASH + "images";
			
			String destinationDirName = PropsUtil.get("static.cache.dir") + StringPool.SLASH + servletContextName + StringPool.SLASH + "images";
			
			FileUtil.copyDirectory(sourceDirName, destinationDirName);
			
			_log.info("Successfuly copy file from " + sourceDirName + " to " + destinationDirName);
		}
	}
	...
	private static final String _TEMP_DIR = SystemProperties.get(SystemProperties.TMP_DIR) + "/liferay/minifier";
