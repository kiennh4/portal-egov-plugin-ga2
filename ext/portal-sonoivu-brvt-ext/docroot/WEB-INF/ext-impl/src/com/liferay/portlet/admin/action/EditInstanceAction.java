...
import com.liferay.portal.license.PortalEgovLicenseUtil;
import com.liferay.portal.license.PortalLicenseException;
...
public class EditInstanceAction extends PortletAction {
	...
	public void processAction(
		...
		try {
			...
			else if (e instanceof PortalLicenseException ||
	...
	protected void updateInstance(ActionRequest actionRequest)
		...
		if (companyId <= 0) {
			...
			/**
			 * Customized by HungDX
			 * Check license limit instances
			 */
			int licenseInstancesLimit = PortalEgovLicenseUtil.getMaxInstances();
			
			if(licenseInstancesLimit > 1){//Acept default instances
				
				int instanceCounter = CompanyLocalServiceUtil.getCompaniesCount();
				
				if(instanceCounter >= licenseInstancesLimit){
					throw new PortalLicenseException("Instances limited by license");
				}
			}
