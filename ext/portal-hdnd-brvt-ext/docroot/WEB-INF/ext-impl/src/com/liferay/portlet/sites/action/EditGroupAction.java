	...
	public void processAction(
		...
		try {
			...
			if (cmd.equals(Constants.ADD) || cmd.equals(Constants.UPDATE)) {
				 ...
				 * Check limit group by license
				 */
				if (cmd.equals(Constants.ADD)){
					int licenseGroupLimit = PortalEgovLicenseUtil.getMaxSites();
					
					if(licenseGroupLimit > 1){//acept default group
						long companyId = PortalUtil.getCompanyId(actionRequest);
						int groupCounter = GroupLocalServiceUtil.getCompanyGroupsCount(companyId);
						
						if(groupCounter >= licenseGroupLimit){
							throw new PortalLicenseException("Group limited by license");
						}
					}
				}
			...
			if (e instanceof NoSuchGroupException ||
				...
				setForward(actionRequest, "portlet.sites_admin.error");
					 ...
					 e instanceof PortalLicenseException) {
