...
<style type="text/css">
	.user-details-infomation{
		padding: 0px;
		margin: 0px;
		display: block;
	}
	
	.user-details-infomation input[type="text"]{
		max-width: 200px !important;
		width: 200px !important;
	}
		
	.user-details-infomation .taglib-logo-selector{
		text-align: left !important;
	}
			
	.user-details-infomation .taglib-logo-selector .lfr-change-logo img{
		text-align: left !important;
...
<div class="user-details-infomation">
	...
	<aui:fieldset column="<%= true %>" cssClass="aui-w50">
		...
		<aui:input name="firstName" label="full-name"/>
		...
		<c:if test="<%= !PrefsPropsUtil.getBoolean(company.getCompanyId(), PropsKeys.USERS_SCREEN_NAME_ALWAYS_AUTOGENERATE) || (selUser != null) %>">
			<c:choose>
				...
				<c:otherwise>
					<aui:input name="screenName" label="account-login-name"/>
				</c:otherwise>
			</c:choose>
		</c:if>
