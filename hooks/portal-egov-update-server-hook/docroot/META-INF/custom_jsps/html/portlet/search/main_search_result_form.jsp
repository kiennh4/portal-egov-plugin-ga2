...
if (assetRendererFactory != null) {//Truong hop la web-content
...
else {//Truong hop kieu du lieu ko phai la web-content (VBPQ,TTHC)
	String portletId = document.get(Field.PORTLET_ID);
	
	//Goi ham tao URL trong file init.jsp
	...
	//Public parameter cua portlet hien thi chi tiet noi dung tuong ung voi kieu du lieu(khai bao trong portlet.xml)
	String assetPublicParameterName = _getAssetPPName(className);
	
	if(Validator.isNotNull(assetPublicParameterName)){//Truong hop assetPublicParameterName != Blank
		
		//Set Id cua entry tuong ung vao URL
		viewFullContentURL.setParameter(assetPublicParameterName, String.valueOf(classPK));			
	}
}
...
<span class="asset-entry">
	...
	<span class="asset-entry-title">
		<a href="<%= viewURL %>">
			<!-- THem doan nay vao de hien thi icon kieu du lieu -->
			<c:choose>
				<c:when test="<%= assetRendererFactory != null %>">
					<img alt="" src="<%= assetRendererFactory.getIconPath(renderRequest) %>" />
				</c:when>
				<c:otherwise>
					<img alt="" src="<%= _getDataTypeIconPath(className) %>" />
				</c:otherwise>
			</c:choose>
