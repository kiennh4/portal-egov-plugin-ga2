...
<div class="aui-helper-hidden update-layout-form" id="<portlet:namespace />addLayout">
	...
	<aui:form action="<%= editPageURL %>" enctype="multipart/form-data" method="post" name="fm2">
		...
		<aui:fieldset>
			...
			<aui:input id="addLayoutName" name="name" label="site-page-name"/>
			...
			<div id="<portlet:namespace />hiddenFields">
				<aui:select id="addLayoutType" name="type" label="site-page-type">
				...
				<c:if test="<%= (selLayout != null) && selLayout.isTypePortlet() %>">
					<aui:input label="copy-parent-page" name="inheritFromParentLayoutId" type="checkbox" />
				</c:if>
			</div>

			<aui:input id="addLayoutHidden" name="hidden" label="hidden-from-navigation-menu"/>
