...
<style type="text/css">
	...
	.portlet-polls .aui-form-validator-message{
		border-radius: 2px;
		width: 302px;
	}
	
	.portlet-polls input[type="text"],
	.portlet-polls textarea,
	.portlet-polls select{
	...
	.portlet-polls input[type="text"]:focus,
	.portlet-polls textarea:focus,
	.portlet-polls select:focus{
...
<aui:form action="<%= editQuestionURL %>" method="post" name="fm" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "saveQuestion();" %>'>
	...
	<liferay-ui:header
		...
		title='<%= (question == null) ? "new-question" : StringUtil.shorten(question.getTitle(locale),80) %>'
	/>
...
<aui:script>
	AUI().ready('aui-base',function(A){
		...
		var pollQuestionTitleInput = A.one('#_25_title_vi_VN');
		
		if(pollQuestionTitleInput){
			pollQuestionTitleInput.focus();
		}
	});
</aui:script>
