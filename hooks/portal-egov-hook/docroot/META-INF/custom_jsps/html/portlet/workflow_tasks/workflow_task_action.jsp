...
<liferay-ui:icon-menu showExpanded="<%= (row == null) %>" showWhenSingleIcon="<%= (row == null) %>">
	...
	<c:if test="<%= _hasOtherAssignees(pooledActorsIds, workflowTask, user) %>">
		...
		<liferay-ui:icon
			...
			image="assign"
			message="workflow-assign-to-user"
			method="get"
			url="<%= assignURL %>"
		/>
	</c:if>
	...
	<c:if test="<%= !workflowTask.isCompleted() %>">
		...
		<liferay-ui:icon
			...
			image="time"
			message="workflow-update-due-date"
			method="get"
...
<div class="aui-helper-hidden" id="<%= randomId %>updateAsignee">
	<c:if test="<%= _hasOtherAssignees(pooledActorsIds, workflowTask, user) %>">
		<aui:select label="assigned-to-user" name="assigneeUserId">
...
<div class="aui-helper-hidden" id="<%= randomId %>updateComments" style="padding-top: 10px;">
	<aui:input cols="60" name="comment" type="textarea" rows="6" label="workflow-task-comment" />
</div>
