...
<style type="text/css">
	...
	.portlet-journal .aui-field-content{
		padding-bottom: 10px;
	}
	
	.portlet-journal .journal-article-general-fields input[type="text"]{
		width: 600px !important;
		max-width: none !important;	
	}
		
	.portlet-journal  input[type="file"]{
	...
	.portlet-journal img.article-image{
		border: 1px solid #e4e4e4;
		padding: 1px;
		width: 100px !important;
	}
				
	.portlet-journal span.cke_skin_kama{
		border: 1px solid #E4E4E4;
    	border-radius: 3px;
    	padding: 0px;
	...
	.portlet-journal .cke_skin_kama .cke_contents{
		min-height: 320px !important;
	}
		
	.portlet-journal .cke_skin_kama .cke_contents iframe{
		min-height: 320px !important;
	}
	
	.portlet-journal .cke_skin_kama .cke_wrapper{
		background-color: #E9E9E9;
	}
	
	.portlet-journal .cke_skin_kama .cke_wrapper table tr{
		height: 20px;
	}	
	
	.portlet-journal .cke_skin_kama .cke_wrapper table tr td{
		padding: 0px;
	}
	...
	.portlet-journal .aui-form-validator-message{
		border-radius: 3px;
		width: 602px;
	}
			
	.portlet-journal .cke_editor textarea{
    	min-width: 700px !important;
		min-height: 400px !important;
	}
...
/**
************************ HUNGDX hook *****************************
* Chuyen cac bien tu section abstract va categorization sang gop vao section content
*/
...
if (Validator.isNull(type)) {
	//HungDX sua loai noi dung mac dinh cua bai viet la Tin tuc
	type = "news";
}
...
<tr>
	<td class="lfr-top">
		...
		<liferay-ui:error exception="<%= ArticleTitleException.class %>" message="please-enter-a-valid-article-title" />
...
<!--***********HungDX hook: Gop noi dung cua section abstract vao section content *********** -->
		<div class="aui-helper-hidden">	
			<aui:select name="type" showEmptyOption="<%= false %>" label="select-asset-type">
		...
		<aui:input label="article-summary-content" languageId="<%= Validator.isNotNull(toLanguageId) ? toLanguageId : defaultLanguageId %>" name="description" />
		...
		<c:if test="<%= Validator.isNull(toLanguageId) %>">
			...
			<span style="font-weight: bold;">
				<aui:input  name="smallFile" type="file" label="small-article-image" inlineLabel="left" cssClass="lfr-input-text-container"/>
			</span>
		</c:if>
		
		<aui:input type="hidden" name="smallImage" value="<%=smallImage %>"/>
		
		<aui:input type="hidden" name="smallImageURL" value="<%=smallImageURL%>"/>
		
		<aui:input type="hidden" name="indexable" value="<%=true %>"/>
								...
								<span class="folder">
									<div class="field-container">
										...
										<label class="journal-article-field-label" for="">
											<span><liferay-ui:message key="article-content" /></span>
										</label>
										...
										%>
											...
											toolbarSet="liferay-article"  width="100%"  />
										<%
											}
											else
											{
										%>
												<script src="/html/js/editor/ckeditor/ckeditor.js"></script>
												<textarea name="<%= renderResponse.getNamespace() + "structure_el_TextAreaField_content" %>" 
													id="<%= renderResponse.getNamespace() + "structure_el_TextAreaField_content" %>" 
													rows="10" cols="80"></textarea>
									            <script>
										            <%
										            String portletIdE = portletDisplay.getRootPortletId();
	
										            String mainPathE = themeDisplay.getPathMain();
	
										            String doAsUserIdE = themeDisplay.getDoAsUserId();
	
										            if (Validator.isNull(doAsUserIdE)) {
										            	doAsUserIdE = Encryptor.encrypt(company.getKeyObj(), String.valueOf(themeDisplay.getUserId()));
										            }
	
										            long doAsGroupIdE = themeDisplay.getDoAsGroupId();
										    		...
										    		sb.append(plid);
										    		sb.append("&p_p_id=");
										    		sb.append(HttpUtil.encodeURL(portletIdE));
										    		sb.append("&doAsUserId=");
										    		sb.append(HttpUtil.encodeURL(doAsUserIdE));
										    		sb.append("&doAsGroupId=");
										    		sb.append(HttpUtil.encodeURL(String.valueOf(doAsGroupIdE)));
		
										    		String connectorURL = HttpUtil.encodeURL(sb.toString());
										    		String name = renderResponse.getNamespace() + "structure_el_TextAreaField_content";
										    		%>
		
										    		CKEDITOR.replace(
										    			'<%= name %>',
										    			{
										    				customConfig: '<%= PortalUtil.getPathContext() %>/html/js/editor/ckeditor/ckconfig.jsp?p_l_id=<%= plid %>&p_p_id=<%= HttpUtil.encodeURL(portletIdE) %>&p_main_path=<%= HttpUtil.encodeURL(mainPathE) %>&doAsUserId=<%= HttpUtil.encodeURL(doAsUserIdE) %>&doAsGroupId=<%= HttpUtil.encodeURL(String.valueOf(doAsGroupIdE)) %>&cssPath=<%= HttpUtil.encodeURL(themeDisplay.getPathThemeCss()) %>&imagesPath=<%= HttpUtil.encodeURL(themeDisplay.getPathThemeImages()) %>&languageId=<%= HttpUtil.encodeURL(LocaleUtil.toLanguageId(locale)) %>',
										    				...
										    				toolbar: '<%= TextFormatter.format(HtmlUtil.escapeJS("liferay-article"), TextFormatter.M) %>'
										    			}
										    		);
									            </script>
										<%
