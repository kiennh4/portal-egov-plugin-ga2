					...
					<div class="article-info">
						<div class="float-container">
							...
							<c:if test="<%= !article.isNew() %>">
								<span class="article-name"><%= HtmlUtil.escape(StringUtil.shorten(article.getTitle(locale), 150)) %></span>
							</c:if>
						</div>
					</div>
				</c:if>
...
<aui:script>
	AUI().ready('aui-base',function(A){
		...
		var articleTitleInput = A.one('#<portlet:namespace />title_vi_VN');
...
private String _getArticleImage(ThemeDisplay themeDisplay, JournalArticle article) {
	...
	if (article.isSmallImage()) {
		if (Validator.isNotNull(article.getSmallImageURL())) {
			...
			try{
				Image smallImage = ImageLocalServiceUtil.getImage(article.getSmallImageId());
				
				if(smallImage != null){
					
					String smallImageIdStr = String.valueOf(article.getSmallImageId());
					...
					stringBuilder.append(article.getSmallImageId() + "." + smallImage.getType());
