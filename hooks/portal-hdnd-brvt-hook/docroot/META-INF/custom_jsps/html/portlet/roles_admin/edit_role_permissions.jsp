...
if (Validator.isNotNull(portletResource)) {
	...
	if (portlet.getPortletId().equals(PortletKeys.PORTAL)) {
		...
		portletResourceLabel = PortalUtil.getPortletLongTitle(portlet, application, Locale.US);
	}
}
		...
		<aui:form action="<%= editRolePermissionsURL %>" method="post" name="fm">
			...
			<c:choose>
				...
				<c:when test="<%= (modelResources != null) && !modelResources.isEmpty() %>">
					...
					<%
					modelResources = ListUtil.sort(modelResources, new ModelResourceComparator(Locale.US));
