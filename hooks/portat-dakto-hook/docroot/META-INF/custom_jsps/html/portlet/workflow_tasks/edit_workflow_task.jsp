...
headerTitle = headerTitle.concat(workflowHandler.getTitle(classPK, locale));
		...
		<aui:layout>
			<aui:column columnWidth="60">
				<div class="lfr-asset-assigned">
					<aui:field-wrapper label="assigned-to-user">
						<c:choose>
							...
							<c:otherwise>
								<liferay-ui:message key="nobody-assigned" />
							</c:otherwise>
						</c:choose>
						...
						<c:if test="<%= !workflowTask.isAssignedToSingleUser() %>">
							...
							<span class="workflow-task task-assign-to-me-link"><aui:a href="<%= assignToMeURL %>" id='<%= randomId + "taskAssignToMeLink" %>' label="workflow-assign-to-me" /></span>
						</c:if>
						...
						<c:if test="<%= _hasOtherAssignees(pooledActorsIds, workflowTask, user) %>">
							...
							<span class="workflow-task task-assign-link"><aui:a href="<%= assignURL %>" id='<%= randomId + "taskAssignLink" %>' label="workflow-assign-to-user" /></span>
						</c:if>
					</aui:field-wrapper>
				</div>
			...
			<aui:column>
				...
				<div class="lfr-asset-due-date">
					<aui:field-wrapper label="due-date">
						<%= (workflowTask.getDueDate() == null) ? LanguageUtil.get(pageContext, "workflow-never-expired") : dateFormatDateTime.format(workflowTask.getDueDate()) %>
