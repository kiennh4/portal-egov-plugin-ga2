	...
	<c:if test='<%=Validator.isNotNull(tthcEntry) %>'>
		...
		<div id="<portlet:namespace/>contentTabs" class="tthc-content-tab">
			...
			<ul id="<portlet:namespace/>tabListNode" class="aui-nav aui-nav-tabs">
				...
				<li><a href="javascript:;"><liferay-ui:message key="tthc-required-document" /></a></li>
		...
		<div class="aui-button-holder">
			<aui:button value="update" name="updateTTHCBtn" />
...
<script type="text/javascript">
	...
	AUI().ready('aui-tabs', 'substitute', function(A) {
		...
		var updateTTHCBtn = A.one('#<portlet:namespace/>updateTTHCBtn');
				
		if(updateTTHCBtn){
			
			updateTTHCBtn.on('click',function(){

				Liferay.Util.getOpener().showTTHCUpdateForm('<%=tthcEntryId %>');
