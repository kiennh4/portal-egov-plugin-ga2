...
<%
	...
	if(departmentId > 0 || categoryId > 0 || doctypeId > 0 ){
	...
	if(advancedSearch){
		...
		vbpqEntryList = VBPQ_EntryLocalServiceUtil.findByMultiCondition(groupId, vbpqAdvancedSearchKeyword, 
															departmentId, categoryId, doctypeId,StringPool.BLANK,StringPool.BLANK,-1,-1,-1);
	}
%>
...
<liferay-portlet:actionURL var="deleteAllURL" name="deletaAllVBPQEntry">
	<portlet:param name="redirectURL" value="<%=vbpqAdminURL%>"/>
		...
		<div class="lfr-header-row-content">						
			...
			<div class="add_new_vbpq_button" align="left">
				...
				<c:if test="<%=VBPQEntryPermission.contains(permissionChecker, groupId, VBPQEntryPermission.FILE_ATTACHMENT_CONFIG) %>">
					<aui:button type="button" name="vbpqUploadSettingsBtn" value="upload-settings" href="<%=vbpqUploadSettingURL %>"/>
				</c:if>
			...
			<div class="vbpq_search_form" align="right">
				<aui:form name="vbpq_search_form" method="post" action="<%=vbpqAdminURL %>">
					...
					<aui:button type="button" value="delete-all" href="<%=deleteAllURL %>"/>
				</aui:form>	
			</div>
		</div>
	</div>
		...
		<liferay-ui:search-container emptyResultsMessage="no-vbpq-entry-found" iteratorURL="<%=vbpqAdminImplURL %>" delta="20" >
			...
			<liferay-ui:search-container-row className="VBPQ_Entry" keyProperty="entryId" modelVar="vbpqEntry" >
				...
				<liferay-ui:search-container-column-text name="#" title='<%=LanguageUtil.get(locale, "view") %>' 
						value="<%=String.valueOf(vbpqEntryIndex) %>" href="<%=rowHREF %>"/>
							
				<liferay-ui:search-container-column-text name="Code" title='<%=LanguageUtil.get(locale, "view") %>' 
						value="<%=vbpqEntry.getEntryCode() %>" href="<%=rowHREF %>"/>
			
				<liferay-ui:search-container-column-text name="vbpq-entry-name" title='<%=LanguageUtil.get(locale, "view") %>'  
						value="<%=vbpqName%>" href="<%=rowHREF %>"/>
	...
	Liferay.provide(
		...
		function(vbpqEntryId) {
			...
			var dialogAlignConfig = {
					node: '#<portlet:namespace/>vbpqUploadSettingsBtn',
					points: ['tl', 'tl']
			};
