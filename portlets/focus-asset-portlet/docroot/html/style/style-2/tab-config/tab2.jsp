...
<liferay-ui:panel collapsible='<%=true%>' extended='<%=true%>' title="2nd-tab-settings" defaultState="closed">
	...
	<aui:input label="style-title" name="secondTabTitle" value="<%=secondTabTitle%>"/>
	...
	<div class="aui-field-row query-row">
		...
		<aui:select inlineField="<%= true %>" name="secondTabQueryContains" label="">
		...
		<aui:select inlineField="<%= true %>" label="" name="secondTabQueryAndOperator">
		...
		<aui:select id='<%= randomNameSpace + "selector" %>' cssClass="tab-asset-query-name"  
					name="secondTabQueryName" label="" inlineField="<%=true %>" inlineLabel="left">
		...
		<div class="aui-field tags-selector <%= Validator.equals(secondTabQueryName, "assetTags") ? StringPool.BLANK : "aui-helper-hidden" %>">
			<liferay-ui:asset-tags-selector
				hiddenInput="secondTabSelectedTagsNames"
		...
		<div class="aui-field categories-selector <%= Validator.equals(secondTabQueryName, "assetCategories") ? StringPool.BLANK : "aui-helper-hidden" %>">
			<liferay-ui:asset-categories-selector
				hiddenInput="secondTabSelectedCategoryIds"
	...
	<aui:select name="secondTabEntriesDisplay" label="total-assets-display" inlineLabel="left">
		<%
	...
	<aui:select inlineField="<%= true %>" name="secondTabOrderByColumn" label="asset-order-by-column" inlineLabel="left" >
	...
	<aui:select inlineField="<%= true %>" label="" name="secondTabOrderByType">
