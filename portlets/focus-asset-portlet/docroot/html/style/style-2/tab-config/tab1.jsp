...
<liferay-ui:panel collapsible='<%=true%>' extended='<%=true%>' title="1st-tab-settings" defaultState="closed">
	...
	<aui:input label="style-title" name="firstTabTitle" value="<%=firstTabTitle%>"/>
	...
	<div class="aui-field-row query-row">
		...
		<aui:select inlineField="<%= true %>" name="firstTabQueryContains" label="">
		...
		<aui:select inlineField="<%= true %>" label="" name="firstTabQueryAndOperator">
		...
		<aui:select id='<%= randomNameSpace + "selector" %>' cssClass="tab-asset-query-name"  
					name="firstTabQueryName" label="" inlineField="<%=true %>" inlineLabel="left">
		...
		<div class="aui-field tags-selector <%= Validator.equals(firstTabQueryName, "assetTags") ? StringPool.BLANK : "aui-helper-hidden" %>">
			<liferay-ui:asset-tags-selector
				hiddenInput="firstTabSelectedTagsNames"
		...
		<div class="aui-field categories-selector <%= Validator.equals(firstTabQueryName, "assetCategories") ? StringPool.BLANK : "aui-helper-hidden" %>">
			<liferay-ui:asset-categories-selector
				hiddenInput="firstTabSelectedCategoryIds"
	...
	<aui:select name="firstTabEntriesDisplay" label="total-assets-display" inlineLabel="left">
		<%
	...
	<aui:select inlineField="<%= true %>" name="firstTabOrderByColumn" label="asset-order-by-column" inlineLabel="left" >
	...
	<aui:select inlineField="<%= true %>" label="" name="firstTabOrderByType">
