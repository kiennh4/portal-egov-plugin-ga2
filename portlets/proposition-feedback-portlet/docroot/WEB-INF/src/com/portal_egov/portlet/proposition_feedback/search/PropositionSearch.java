...
import com.portal_egov.portlet.proposition_feedback.comparator.PropositionComparatorUtil;
import com.portal_egov.portlet.proposition_feedback.model.Proposition;


public class PropositionSearch extends SearchContainer<Proposition>{
	
	public static final String EMPTY_RESULTS_MESSAGE = "no-entry-were-found";
	
	public PropositionSearch(PortletRequest portletRequest,PortletURL iteratorURL){
		...
		iteratorURL.setParameter(PropositionDisplayTerms.PROPOSITION_NAME, displayTerms.getName());
		...
		try {
			...
			OrderByComparator orderByComparator = PropositionComparatorUtil.getPropositionOrderByComparator(orderByCol, orderByType);
	...
	private static Log _log = LogFactoryUtil.getLog(PropositionSearch.class);
}
