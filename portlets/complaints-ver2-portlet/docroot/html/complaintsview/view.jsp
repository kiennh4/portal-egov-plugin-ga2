...
<%
	...
	List<Complaints> searchResults = ComplaintsLocalServiceUtil.findByKeyWord(groupId, searchKeyword, -1, -1, null);  
%>
	...
	<div class="complaint-search-result">
		...
		<liferay-ui:search-container iteratorURL="<%=complaintViewPageImplURL %>" emptyResultsMessage="no-data">
			...
			<liferay-ui:search-container-row className="Complaints" keyProperty="complaintId" modelVar="complaint" escapedModel="<%=true %>">
				...
				sb.append(PortalUtil.getPortletNamespace(portletId)+"showComplaintPreviewDialog");
				sb.append("('");
				...
				<liferay-ui:search-container-column-text 
					name="number" 
					value="<%=complaint.getComplaintNumber()%>" 
					href="<%=rowHREF %>"
					title="view"
				/>
				...
				<liferay-ui:search-container-column-text 
					name="signing-date" 
					value="<%=simpleDateFormat.format(complaint.getSigningDate())%>"
					href="<%=rowHREF %>"
					title="view" 
				/>
...
<script type="text/javascript">
	...
	Liferay.provide(
		window,
		'<portlet:namespace/>showComplaintPreviewDialog',
		function(complaintId,complaintTitle) {
			...
			if(complaintPreviewRenderURL){
				...
				complaintPreviewRenderURL.setParameter('jspPage', '/html/complaintsview/preview.jsp');
