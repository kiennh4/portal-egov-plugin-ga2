...
public class LegalFAQ extends MVCPortlet {
	...
	public void updateAskContent(ActionRequest actionRequest,
		...
		try {
			...
			String citizenName = HtmlUtil.stripHtml(ParamUtil.getString(
					actionRequest, "citizenName", StringPool.BLANK));

			String citizenPhone = HtmlUtil.stripHtml(ParamUtil.getString(
					actionRequest, "citizenPhone", StringPool.BLANK));

			String citizenAddress = HtmlUtil.stripHtml(ParamUtil.getString(
					actionRequest, "citizenAddress", StringPool.BLANK));

			String citizenEmail = HtmlUtil.stripHtml(ParamUtil.getString(
					actionRequest, "citizenEmail", StringPool.BLANK));

			String askTitle = HtmlUtil.stripHtml(ParamUtil.getString(
					actionRequest, "askTitle", StringPool.BLANK));

			String askContent = HtmlUtil.stripHtml(ParamUtil.getString(
					actionRequest, "askContent", StringPool.BLANK));
			...
			ServiceContext serviceContext = ServiceContextFactory.getInstance(
					LegalFAQEntry.class.getName(), actionRequest);

			serviceContext
					.setGroupPermissions(new String[] { LegalFAQEntryPermission.VIEW });
			serviceContext
					.setGuestPermissions(new String[] { LegalFAQEntryPermission.VIEW });

			if (checkCaptcha(actionRequest)) {
				LegalFAQEntryLocalServiceUtil.addFAQEntry(companyId, groupId,
						userId, categoryId, citizenName, citizenPhone,
						citizenEmail, citizenAddress, now, askTitle,
						askContent, now, StringPool.BLANK,
						LegalFAQConstants.LEGAL_FAQ_ENTRY_PUBLISH_STATUS_VALUE,
						LegalFAQConstants.LEGAL_FAQ_ENTRY_DEFAULT_STATUS_VALUE,
						serviceContext);

				// String emailContent =
				// FAQEmailUtil.buildFAQEmailContent(faqEntry);
	...
	public void updateEntryViewCount(ActionRequest actionRequest,
			ActionResponse actionResponse) {
		try {

			long legalFAQEntryId = ParamUtil.getLong(actionRequest,
					"legalFAQEntryId", 0L);

			if (legalFAQEntryId > 0) {

				LegalFAQEntry legalFAQEntry = LegalFAQEntryLocalServiceUtil
						.getEntry(legalFAQEntryId);

				legalFAQEntry.setViewCount(legalFAQEntry.getViewCount() + 1);

				LegalFAQEntryLocalServiceUtil.updateLegalFAQEntry(
						legalFAQEntry, false);
			}
		} catch (Exception e) {
			_log.error(e);
