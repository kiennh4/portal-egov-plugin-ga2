...
//import com.portal_egov.portlet.vbpq.VBPQManagement;
...
import com.bkav.portal.portlet.contentsharing.util.vbpq.FileAttachmentUtil;
...
public class FileAttachmentUtil {
	 ...
	 */
		 ...
		 * @return
		 */
		public static List<DLFileEntry> getVBPQFileAttachment(long vbpqEntryId){
			
			try {
				List<DLFileEntry> vbpqFileAttachmentList = new ArrayList<DLFileEntry>();
				
				List<VBPQ_DlFileEntries> vbpqDLFileEntriesList = VBPQ_DlFileEntriesLocalServiceUtil.findByVBPQEntry(vbpqEntryId); 
				
				for(int i = 0; i < vbpqDLFileEntriesList.size();i++){
					
					VBPQ_DlFileEntries vbpqDLFileEntries = vbpqDLFileEntriesList.get(i);
					
					DLFileEntry vbpqFileAttachment = getDLFileEntry(vbpqDLFileEntries.getFileEntryId());
					
					if(vbpqFileAttachment != null){
						
						vbpqFileAttachmentList.add(vbpqFileAttachment);
					}else{
						
						VBPQ_DlFileEntriesLocalServiceUtil.deleteVBPQ_DlFileEntries(vbpqDLFileEntries);
					}
				}
				
				return vbpqFileAttachmentList;
			}
			catch (Exception e) {			
		...
		public static long createAttachmentFolder(long userId,long groupId,long repositoryId, long parentFolderId,
			...
			long attachmentFolderId = VBPQConstants.DL_ROOT_FOLDER_ID;
		...
		private static final String _TEMP_FOLDER_NAME = FileAttachmentUtil.class.getName();
