AUI.add(
	'portal-egov-tourism-map-admin-portlet',
	function(A) {
		...
		var UPDATE_GROUP_COMMAND = 'update-group';
		
		var ADD_NEW_GROUP_PANEL_TITLE = 'add-new-marker-group';
		
		var UPDATE_GROUP_PANEL_TITLE = 'update-marker-group';
		
		var UPDATE_MARKER_COMMAND = 'update-marker';
		
		var ADD_NEW_MARKER_PANEL_TITLE = 'add-new-marker';
		
		var UPDATE_MARKER_PANEL_TITLE = 'update-marker';
		
		var MAP_CONFIG_COMMAND = 'map-config';
		
		var MAP_CONFIG_PANEL_TITLE = 'map-config';
		...
		var MAP_CONFIG_BTN_NAME = 'mapConfigBtn';
		
		var ADD_GROUP_BTN_NAME = 'addGroupBtn';
		
		var ADD_MARKER_BTN_NAME = 'addMarkerBtn';
		
		var CONFIRM_DELETE_GROUP_MESSAGE = 'are-you-sure-want-to-delete-this-marker-group?';
		
		var CONFIRM_DELETE_MARKER_MESSAGE = 'are-you-sure-want-to-delete-this-marker?';
		...
		var GROUP_ITEM_TEMPLATE 	= '<li class="group-item results-row {cssClassSelected}" data-markerGroupId="{markerGroupId}">'
									...
									+ 			'<a class="group-item-edit-trigger" href="javascript:;" data-markerGroupId="{markerGroupId}"></a>'
									+ 			'<a class="group-item-delete-trigger" href="javascript:;" data-markerGroupId="{markerGroupId}"></a>'
									+ 		'</span>'	
									+ 	'</div>'
									+ '</li>';
		...
		var MARKER_ITEM_TEMPLATE 	= '<li class="marker-item results-row" data-markerId="{markerId}">'
									...
									+ 			'<a class="marker-item-edit-trigger" href="javascript:;" data-markerId="{markerId}"></a>'
									+ 			'<a class="marker-item-delete-trigger" href="javascript:;" data-markerId="{markerId}"></a>'
									+ 		'</span>'	
									+ 	'</div>'
									+ '</li>';
		
		var TourismMapAdmin = A.Component.create(
			{
				EXTENDS: A.Base,
				NAME: 'tourismmap',
				prototype: {
					...
					initializer: function(config) {
						...
						instance.portletId = 'tourism_map_admin_WAR_tourismmapportlet';
						
						instance.tourismMap = null;
						
						instance.markerArray = [];
						
						instance.adminPanel = null;
						...
						var idPrefix = '#_' + instance.portletId + '_';
						
						var mapConfigBtn = A.one(idPrefix + MAP_CONFIG_BTN_NAME);
						if(mapConfigBtn){
							mapConfigBtn.on(EVENT_CLICK,function(){
								instance._showDialogPanel(MAP_CONFIG_COMMAND,MAP_CONFIG_PANEL_TITLE);
							});
						}
						
						var addGroupBtn = A.one(idPrefix + ADD_GROUP_BTN_NAME);
						
						if(addGroupBtn){
							addGroupBtn.on(EVENT_CLICK,function(){
								
								instance._showDialogPanel(UPDATE_GROUP_COMMAND,ADD_NEW_GROUP_PANEL_TITLE);
							});
						}
						
						var addMarkerBtn = A.one(idPrefix + ADD_MARKER_BTN_NAME);
						if(addMarkerBtn){
							addMarkerBtn.on(EVENT_CLICK,function(){
								instance._showDialogPanel(UPDATE_MARKER_COMMAND,ADD_NEW_MARKER_PANEL_TITLE);
							});
						}
					...
					_onGroupMenuClick : function(event){
						...
						if(selectedGroup){
							...
							if(event.target.hasClass('group-item-edit-trigger')) {//Edit group
								
								instance._selectedGroupId = selectedGroupId;
								
								instance._showDialogPanel(UPDATE_GROUP_COMMAND,UPDATE_GROUP_PANEL_TITLE);
									
							}else if(event.target.hasClass('group-item-delete-trigger')) {//Delete Group
									
								if(instance._confirmDelete(CONFIRM_DELETE_GROUP_MESSAGE)){
									
									instance._deleteGroup(selectedGroupId);
									
									instance._showGroupsMenu();
								}
							}else{//Hiển thị danh sách các điểm đánh dấu thuộc group
					...
					_onMarkerClick : function(event){
						...
						if(selectedMarkerItem){
							...
							if(event.target.hasClass('marker-item-edit-trigger')){//Cập nhật địa điểm
								
								instance._selectedMarkerId = selectedMarkerId;
								
								instance._showDialogPanel(UPDATE_MARKER_COMMAND,UPDATE_MARKER_PANEL_TITLE);
								
							}else if (event.target.hasClass('marker-item-delete-trigger')){//Xóa địa điểm
								
								if(instance._confirmDelete(CONFIRM_DELETE_MARKER_MESSAGE)){
									
									instance._deleteMarker(selectedMarkerId);
									
									instance._showAllMarkers();
								}
								
							}else{//Hiển thị thông tin địa điểm
					...
					_deleteGroup : function(markerGroupId,callback){
						
						Liferay.Service.tourismmap.TourismMapMarkerGroup.deleteMarkerGroup(
							{
								markerGroupId: markerGroupId
							},
							function(result){
					...
					_deleteMarker : function(markerId,callback){
						
						Liferay.Service.tourismmap.TourismMapMarker.deleteMarker(
							{
								markerId: markerId
							},
							function(result){
					...
					/**
					 * Hiển thị Dialog thêm mới/cập nhật nhóm,điểm đánh dấu
					 * @param actionCommand
					 * @param dialogTitle
					 */
					_showDialogPanel : function(actionCommand,dialogTitle){
						
						var instance = this;
						
						instance._hideDialogPanels();
						
						var dialogURL = instance._createRenderURL(actionCommand);
						
						if(!instance.adminPanel){
														
							var dialogAlignConfig = {
									node: '#_tourism_map_admin_WAR_tourismmapportlet_mapConfigBtn',
									points: ['tl', 'tl']
							};
							
							instance.adminPanel = new A.Dialog({
									align: dialogAlignConfig,
									title: Liferay.Language.get(dialogTitle),
									resizable: true,
									destroyOnClose:true,
									width: 365,
									zIndex: 1000
							}).render();
							instance.adminPanel.plug(
								A.Plugin.IO,{
									autoLoad: false,
									uri: dialogURL.toString()
								}
							);
							instance.adminPanel.hide();
						}
						instance.adminPanel.show();
						instance.adminPanel.io.start();
					},
					 ...
					 * @returns
					 */
					_createRenderURL : function(actionCommand){
						...
						if(renderURL){
							...
							if(actionCommand == UPDATE_GROUP_COMMAND){
								...
								renderURL.setParameter('markerGroupId', instance._selectedGroupId);
								
								instance._selectedGroupId = 0;
								
								jspPath = '/html/admin/marker_group_update_form.jsp';
							
							}else if(actionCommand == UPDATE_MARKER_COMMAND){
								
								renderURL.setParameter('markerId', instance._selectedMarkerId);
								
								instance._selectedMarkerId = 0;
								
								jspPath = '/html/admin/marker_update_form.jsp';
								
							}else if(actionCommand == MAP_CONFIG_COMMAND){
								jspPath = '/html/admin/map_config.jsp';
							}
					...
					/**
					 * Ẩn tất cả những cửa sổ Dialog đang hiển thị.
					 */
		...
		Liferay.Portlet.TourismMapAdmin = TourismMapAdmin;
	},
	'',
	{
