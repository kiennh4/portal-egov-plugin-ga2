...
<liferay-ui:header title="select-ykct-upload-folder" backURL="<%=ykctAdminURL%>" />

<c:choose>
	<c:when test="<%=YKCTEntryPermission.contains(permissionChecker, groupId, YKCTEntryPermission.FILE_ATTACHMENT_CONFIG) %>">			
		<%	
			...
			long ykctAttachmentFolderId = GetterUtil.getLong(preferences.getValue("ykctAttachmentFolderId", "0"));
			...
			List<DLFolder> dlFolderList = DLFolderLocalServiceUtil.getFolders(groupId, 0L);	
		%>
		...
		<div class="ykct-settings-form" >
			
			<aui:form name="ykct_settings_form" method="post" action="<%=savePortletConfigURL %>">
				
				<div class="lfr-form-row lfr-form-row-inline">
					<aui:select name="ykctAttachmentFolderId" label="select-folder">				
						<aui:option value="<%=DLFolderConstants.DEFAULT_PARENT_FOLDER_ID %>" label="select" cssClass="root-folder"
									selected="<%=(ykctAttachmentFolderId == DLFolderConstants.DEFAULT_PARENT_FOLDER_ID) %>"
						/>					
						<%
						...
						%>
							<aui:option value="<%=dlFolder.getFolderId()%>" label="<%=dlFolder.getName() %>" cssClass="root-folder"
										selected="<%=dlFolder.getFolderId() == ykctAttachmentFolderId%>"
										
							/>
							
						<%
							buildDLFolderTree(groupId, dlFolder.getFolderId(),1, out, ykctAttachmentFolderId);
						}
						%>					
					</aui:select>
				...
				<aui:input type="checkbox" id="autoCreateSubFolder" name="autoCreateSubFolder" value="<%=autoCreateSubFolder %>" 
							label="auto-create-upload-folder-for-each-ykct-entry"/>	
				
				<div class="aui-button-holder">
					<aui:button type="submit"/>
					<aui:button type="button" value="cancel" href="<%=ykctAdminURL %>"/>
				</div>	
			</aui:form>
		</div>
...
<script type="text/javascript">
	...
	AUI().ready('aui-base','aui-form-validator','aui-overlay-context-panel',function(A){
		...
		window.validator = new A.FormValidator({
	        ...
	        boundingBox: document.<portlet:namespace />ykct_settings_form,
	        rules: {
